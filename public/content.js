function saveConversation() {
  ...
  function showNotification(message, backgroundColor = '#10a37f') {
    ...
    if (backgroundColor === '#10a37f') { // Success
        icon.innerHTML = `
            <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
            <polyline points="22 4 12 14.01 9 11.01"></polyline>
        `;
    } else { // Warning/Already exists
        icon.innerHTML = `
  ...
  chrome.storage.local.get([`buttonPosition_${window.location.hostname}`], function(result) {
    const position = result[`buttonPosition_${window.location.hostname}`] || { top: '20px', left: '20px' };
  ...
  function drag(e) {
    if (isDragging) {
        ...
        currentX = Math.min(Math.max(0, currentX), window.innerWidth - saveButton.offsetWidth);
        currentY = Math.min(Math.max(0, currentY), window.innerHeight - saveButton.offsetHeight);
  ...
  saveButton.addEventListener('mouseover', () => {
    saveButton.style.background = '#5a4972';
  });
  saveButton.addEventListener('mouseout', () => {
    saveButton.style.background = '#705e8a';
  });
